 ...
 * Evaluates default hardcoded rules first, but falls back to dynamic 
 * score calculation if a user logs a new interaction.
 */
export function analyzeCustomerIntelligenceSync(
  customer: Customer,
  contacts: Contact[],
  ...
  // 1. Check if the latest interaction is a custom entry added dynamically
  const isCustomInteraction = latest?.id.startsWith('int_');

  // 2. Default hardcoded heuristic matches for seed data
  ...
  // 3. Dynamic local fallback for newly added notes/interactions
  const latestNote = latest?.notes || 'No recent notes.';
  return {
    attentionScore: 90, // Boost score to top of queue when new action is logged
    urgencyReason: `New ${latest?.type || 'interaction'} logged on ${latest?.occurred_at || 'today'}.`,
    summary: `Latest update: "${latestNote}" (${custInteractions.length} total touchpoints).`,
    keyPainPoints: ["Recent follow-up", "Pending response"],
    suggestedAction: `Follow up with ${primaryContact?.name || 'client'} regarding recent ${latest?.type || 'note'}.`,
    recommendedEmailDraft: `Hi ${primaryContact?.name || 'there'},\n\nFollowing up on our recent conversation regarding: "${latestNote}".\n\nPlease let me know if you need any additional details!\n\nBest regards,`
  };
}

/**
 * Async API caller function.
 * Attempts live Gemini API inference via Next.js Route Handler,
 * falling back to analyzeCustomerIntelligenceSync if offline.
 */
export async function analyzeCustomerIntelligenceAsync(
  customer: Customer,
  contacts: Contact[],
  interactions: Interaction[]
  ...
  try {
    const res = await fetch('/api/ai/analyze', {
      ...
      body: JSON.stringify({ customer, contacts, interactions }),
    });
    ...
    console.warn('Gemini API endpoint unavailable; using local engine fallback:', err);
    return analyzeCustomerIntelligenceSync(customer, contacts, interactions);
  }
}

// Export default alias for synchronous compatibility
export const analyzeCustomerIntelligence = analyzeCustomerIntelligenceSync;